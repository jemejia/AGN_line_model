...
xmax=9000
n_spec=1
#sp_to_run=sp_to_run=[1,3,4,8,9,11,15,16,18,20,22,24,27]
...
for i in sp_to_run:
    ...
    for w in spec_division:
        ...
        if w=="UV": 
            continuous,wlmin_UV=continuous_substraction( i, sp, mag_order,UV_limits)
            backup=sp.copy()
            sp.data=sp.data - continuous
            balmer_template,balmer_tot, index=balmer_normalization(sp)
            ...
            cont,balmer_template=total_continuous_fit(sp,cont,balmer_template,mag_order)
            continuous=cont.data
            balmer_tot=balmer_template.data
            k=0
        ...
        if w=="FUV": 
            sp.data[:index]=sp.data[:index] - balmer_template.data[:index] 
            continuous,wlmin_FUV=continuous_substraction( i, sp, mag_order,FUV_limits)
        
        if w=="OP": continuous,wlmin_OP=continuous_substraction( i, sp, mag_order,OP_limits)
        #-------------continuous subtraction-------------------#
        ...
        if w=="UV":
            ...
            template_file="./templates/fe2_UV3.dat"
            ...
            fe_fit=blue_bump_fitter(sp,"SBB",i,xmin_fe,xmax_fe,fe_template_UV,mag_order)
            pylab.rcParams["figure.figsize"]=8,6            
    ...
    pylab.ylim(ymin=-1.0*np.abs(copy1.data.min()),ymax=1.05*copy1.data.max())
    ...
    copy1=sp.copy()
    copy1.crop(1400,1600)
    pylab.ylim(ymin=-1.3*np.abs(sp.data.min()),ymax=1.1*copy1.data.max())
    pylab.xlim(xmin=sp.xarr[0],xmax=sp.xarr[-1])
    pylab.ylabel(r'$10^{'+str(mag_order)+'}$ erg s$^{-1}$  $cm^{-1}$')
    pylab.xlabel(r'$\AA$')
    pylab.xscale('log')
    pylab.plot(sp.xarr - total,sp.data,'k')
