...
for i in sp_to_run:
    ...
    Halpha_fit=line_fitter("Halpha", i, guesses_Halpha, limits_Halpha, limited_Halpha, tied_Halpha, xmin_Halpha, xmax_Halpha,offset=-0.05, magorder=mag_order)
    ...
    pylab.figure()
    pylab.ylim(ymin=-1.3*np.abs(copy1.data.min()),ymax=1.1*copy1.data.max())
    pylab.xlim(xmin=2000,xmax=4000)
    pylab.ylabel(r'$10^{-'+str(mag_order)+'}$ erg s$^{-1}$  $cm^{-1}$')
    pylab.xlabel(r'$\AA$')
    pylab.xscale('log')
