...
def hbeta_fitter(line_name,spec_number,xmin,xmax,fe_template):
    ...
    def model(xarr, scale_fe,shift_fe,delta,smooth_factor_fe):#,scale_balmer):,fe_template=fe_template.copy(), balmer_template=balmer_template.copy()):        
        sigma=kms_to_wl(900,4861) 
        new_fe_template = scale_fe*pyspeckit.interpolation._interp(xarr.as_unit('angstroms'), 
                                                                   ...
                                                                   gauss_conv(fe_template.data, smooth_factor_fe*sigma, order=0, output=None, mode='reflect', cval=0.0) ) + delta
    ...
    modelclass = pyspeckit.spectrum.models.model.SpectralModel(model,
                                                                   4, 
                                                                   parnames=['scale_fe','shift_fe','delta','smooth'],#,'scale_balmer'], 
                                                                   parlimited=[(True,False),(False,False),(False,False),(True,True)],#,(False,False)],  
                                                                   parlimits=[(0.05,10.0), (-10.0,10.0),(-0.5,0.5),(3.0,12.0)],#,(0.9,1.1)], 
                                                                   shortvarnames=(r'Af',r'\Delta xf',r'delta y','smooth'),#,r'Ab'),
    ...
    sp.Registry.add_fitter('template',modelclass,4,multisingle='multi')
    sp.specfit(fittype='template',exclude=exclude,guesses=[0.1,0.0, -0.1,3.0],xmin=xmin,xmax=xmax)
    print "sp1 lenght=", len(sp.data)
    sp.specfit.fullsizemodel()  
    print "sp2 lenght=", len(sp.data)
    print sp.specfit.parinfo.values[0]
    print sp.specfit.parinfo.values[1]
    print sp.specfit.parinfo.values[2]
    print sp.specfit.parinfo.values[3]
    fe_template.data*=sp.specfit.parinfo.values[0]
    fe_template.data+=sp.specfit.parinfo.values[2]
    ...
    def suavizado(xarr, smooth_factor_fe,template=new_template.copy()):#,scale_balmer):,fe_template=fe_template.copy(), balmer_template=balmer_template.copy()):
        ...
        sigma=kms_to_wl(900,4861) 
        #template.data=gauss_conv(template.data, sigma*smooth_factor_fe, order=0, output=None, mode='reflect', cval=0.0)
    
        n_template =pyspeckit.interpolation._interp(xarr.as_unit('angstroms'), 
                                                    template.xarr.as_unit('angstroms'),
                                                    gauss_conv(template.data, smooth_factor_fe*sigma, order=0, output=None, mode='reflect', cval=0.0) )
        #gauss_conv(template.data, sigma*smooth_factor_fe, order=0, output=None, mode='reflect', cval=0.0))
    ...
    sp.specfit(fittype='suavizado',exclude=exclude,guesses=[3.0],xmin=xmin,xmax=xmax)
    print sp.specfit.parinfo.values[0], "   suavizado"
    # -----------------  fitting ---------------------
    ...
    pylab.plot(fe_template.xarr,fe_template.data,'pink')
...
def continuous_substraction( spec_number, magorder=51):
...
def blue_bump_fitter(line_name,spec_number,xmin,xmax,fe_template,balmer_template):
    ...
    for i in range(5):
        ...
        def model(xarr, scale_fe,shift_fe):#,scale_balmer):,fe_template=fe_template.copy(), balmer_template=balmer_template.copy()):
        ...
        modelclass = pyspeckit.spectrum.models.model.SpectralModel(model,
                                                                   ...
                                                                   parlimited=[(True,True),(False,False)],#,(False,False)],  
                                                                   parlimits=[(0.1,10.0), (-10.0,10.0)],#,(0.9,1.1)], 
        ...
        sp.Registry.add_fitter('template',modelclass,2,multisingle='multi')
        sp.specfit(fittype='template',exclude=exclude,guesses=[1.0,0,0.0],xmin=xmin,xmax=xmax)
        print "sp1 lenght=", len(sp.data)
        sp.specfit.fullsizemodel()  
        print "sp2 lenght=", len(sp.data)
        fe_template.data*=sp.specfit.parinfo.values[0]
...
def line_fitter(line_name, spec_number,guesses, limits, limited, tied, xmin, xmax, offset=-0.4, magorder=-16):
    ...
    # -----------------  line measuring  ---------------------
    sp.measure(z = 0.00, fluxnorm = 10**(mag_order))
    # ----------------- TO DEVELOP!!!!  ---------------------
...
for i in sp_to_run:
    ...
    template_file="./templates/balmer_template3.txt"
    balmer_lines_file="./templates/balmer_lines3.txt"
    ...
    #Mg_fit=line_fitter("MgII", i, guesses_MgII, limits_MgII, limited_MgII, tied_MgII, xmin_MgII, xmax_MgII,offset=-0.05, magorder=mag_order)
    ...
    #fe_fitter("C",i,xmin_CIII,xmax_CIII)
    #Si_fit=line_fitter("SiIV", i,guesses_SiIV, limits_SiIV, limited_SiIV, tied_SiIV, xmin_SiIV, xmax_SiIV, magorder=mag_order)
    #print "FWHM_Si=", sp.specfit.measure_approximate_fwhm()
    #C_fit=line_fitter("C", i,guesses_C, limits_C, limited_C, tied_C, xmin_C, xmax_C,magorder=mag_order)
    #print "FWHM_C=", sp.specfit.measure_approximate_fwhm()
    xmin_fe=4440
    xmax_fe=5250
    template_file="./templates/fe2_Op.dat"
    fe_template = pyspeckit.Spectrum(template_file)
    index_max=np.argmin( np.abs( 5100 - fe_template.xarr) )
    #fe_template.data=fe_template.data[:index_max]
    #fe_template.xarr=fe_template.xarr[:index_max]
    fe_template.xarr.units='angstroms'
    fe_template.xarr.xtype='wavelength'
    lambda_fe1=4500
    lambda_fe2=4720
    index_sp1=np.argmin( np.abs( lambda_fe1 - sp.xarr) )
    index_fe1=np.argmin( np.abs( lambda_fe1 - fe_template.xarr) )
    index_sp2=np.argmin( np.abs( lambda_fe2 - sp.xarr) )
    index_fe2=np.argmin( np.abs( lambda_fe2 - fe_template.xarr) )
    flux=np.mean(sp.data[ index_sp1 : index_sp2 ])
    flux_fe=np.mean(sp.data[ index_fe1 : index_fe2 ])
    fe_template.data*=np.abs(flux/(8*flux_fe))
    print fe_template.data
    fe_fit=hbeta_fitter("optical",i,xmin_fe,xmax_fe,fe_template)
    
    #print limits_Hbeta
    Hbeta_fit=line_fitter("Hbeta", i, guesses_Hbeta, limits_Hbeta, limited_Hbeta, tied_Hbeta, xmin_Hbeta, xmax_Hbeta,offset=-0.05, magorder=mag_order)
    print "FWHM_Hbeta=", sp.specfit.measure_approximate_fwhm()
    #Halpha_fit=line_fitter("Halpha", i, guesses_Halpha, limits_Halpha, limited_Halpha, tied_fe_template.data=fe_template.data[:index_max]Halpha, xmin_Halpha, xmax_Halpha,offset=-0.05, magorder=mag_order)
    ...
    total=continuous  #+ fe_fit  + balmer_data + Mg_fit

    pylab.rcParams["figure.figsize"]=16,6

    total=continuous  + fe_fit  + Hbeta_fit#balmer_data + Mg_fit
    pylab.figure()
    pylab.ylim(ymin=-1.3*np.abs(copy1.data.min()),ymax=1.1*copy1.data.max())
    ...
    pylab.ylim(ymin=-1.2*np.abs(copy.data[index_min]),ymax=1.1*copy.data\
[index_max])
    ...
    plot_file="./plots/Hbeta+fe_" + str(i) + ".png"
    #sp.plotter.figure.savefig(plot_file)
    pylab.savefig(plot_file)
