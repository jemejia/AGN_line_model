...
for i in sp_to_run:
    ...
    #----------starting plotter--------------#
    #copy.plotter(xmin=xmin,xmax=6900,ymin=-1.3*np.abs(copy1.data.min()),ymax=1.1*copy1.data.max())
    #copy3.plotter(xmin=xmin,xmax=6900,ymin=-1.3*np.abs(copy2.data.min()),ymax=1.1*copy1.data.max())
    #----------starting  plotter--------------#


    #-------------continuous fitting regions file-------------------#
    ...
    sp.plotter(xmin=xmin,xmax=xmax,ymin=1.1*copy1.data.min(),ymax=3.5*copy1.data.max())
    ...
    pylab.figure()
    pylab.ylim(ymin=copy1.data.min(),ymax=1.1*copy1.data.max())
    pylab.xlim(xmin=wlmin,xmax=wlmax)
    pylab.ylabel(r'$10^{'+str(mag_order)+'}$ erg s$^{-1}$  $cm^{-1}$')
    pylab.xlabel(r'$\AA$')
    pylab.yscale('linear')
    pylab.plot(sp.xarr,sp.data,'k')#copy2
