...
def total_continuous_fit(spec_number,sp,continuous,balmer,magorder,region):
...
def blue_bump_fitter(sp,line_name,spec_number,xmin,xmax,fe_template, magorder, do_fit=True):
    if do_fit:
    ...
    # -----------------  ploting  fit---------------------
        ...
        plot_file="./plots/fe_fit_" + line_name + "_" + group[spec_number]+"_"+ name[spec_number] + ".png"
        pylab.figure()
        pylab.ylim(ymin=-1.2*np.abs(sp.data[fitmin:fitmax].min()),ymax=2*sp.specfit.residuals[fitmin:fitmax].max())
        pylab.xlim(xmin=xmin-300,xmax=xmax)
        pylab.ylabel(r'$10^{'+str(magorder)+'}$ erg s$^{-1}$  $cm^{-1}$')
       ...
       # ----------plotting residuals-------------
        ...
        plot_file="./plots/" + line_name +"_res_" + group[spec_number]+"_"+ name[spec_number] + ".png"
        pylab.savefig(plot_file)
        #pylab.close()
        # ----------plotting residuals-------------#
...
def line_fitter(sp,line_name, spec_number,guesses, limits, limited, tied, xmin, xmax, offset=-0.4, magorder=-16, do_fit=True):
    if do_fit:
        ...
        # ----------plotting residuals-------------
        plot_file="./plots/" + line_name + "_" + group[spec_number]+"_"+ name[spec_number] + ".png"
        ...
        filename="./plots/" + line_name + "_" + group[spec_number]+"_"+ name[spec_number] + ".txt"
        f=open(filename,"w")
...
def fe_fitter(sp,line_name,spec_number,xmin,xmax,template,magorder,do_fit=True):
    if do_fit:
    #-----------------------------defining fe template fitter ------------------------------------#
