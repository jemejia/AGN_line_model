...
def continuous_substraction( spec_number, magorder=51):
    ...
    cont=exclude_cont[spec_number,:][1:7]
...
def blue_bump_fitter(line_name,spec_number,xmin,xmax,fe_template,balmer_template):
    ...
    for i in range(2):
        ...
        modelclass = pyspeckit.spectrum.models.model.SpectralModel(model,
                                                                   ...
                                                                   parlimited=[(True,False),(False,False),(True,True)],#,(False,False)],  
                                                                   parlimits=[(0,10.0), (-50.0,50.0),(1.0,5.0)],#,(0.9,1.1)], 
        ...
        sp.specfit(fittype='template',exclude=exclude,guesses=[1.0,0,2.0],xmin=xmin,xmax=xmax)
        ...
        sp.specfit(fittype='suavizado',exclude=exclude,guesses=[2.0],xmin=xmin,xmax=xmax)
...
d_center=2000 #km/s red-blue shift allowed to the center of each line
d_center_n=100
em_la=0.0
em_ua=1.0
ab_la=-1.0
...
for i in sp_to_run:
    ...
    fe_template.xarr.xtype='wavelength'
    lambda_fe=4400
    fe_template.xarr=fe_template.xarr + 26.0
    index_sp=np.argmin( np.abs( lambda_fe - sp.xarr) )
    index_fe=np.argmin( np.abs( lambda_fe - fe_template.xarr) )
    flux=np.mean(sp.data[ index_sp - 50 : index_sp + 50 ])
    flux_temp=np.mean(fe_template.data[ index_fe - 50 : index_fe + 50 ])
    fe_template.data*=0.8*flux/(15.0*flux_temp)
    fe_template.data=smooth(fe_template.data,4.0*smooth_f , downsample=False)
    ...
    pylab.figure()
    pylab.ylim(ymin=-1.3*np.abs(copy1.data.min()),ymax=1.1*copy1.data.max())
    pylab.xlim(xmin=4000,xmax=5000)
    pylab.ylabel(r'$10^{-'+str(mag_order)+'}$ erg s$^{-1}$  $cm^{-1}$')
    pylab.xlabel(r'$\AA$')
    pylab.xscale('log')
    pylab.plot(sp.xarr,copy3.data,'k')
    pylab.plot(fe_template.xarr,fe_template.data+0.26,"pink")
